...
# Списки могут быть объявлены при помощи конструктора list():
contacts = list()
...
# Для замены значений используется обращение по индексу 
# с последующим присвоением нового значения.
...
print()
# При удалении списка с помощью del, или сборщика мусора, его пустой объект кешируется и используется в последующем для создания нового списка.
